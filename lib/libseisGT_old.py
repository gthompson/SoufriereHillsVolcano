...
'''
"""
def simple_clean(tr, taperFraction=0.05, filterType="highpass", freq=[0.005], \
...
def process(st_or_tr, func): # probably need to add **kwargs to make this useful
...
def preprocess_trace(tr, bool_despike=True, bool_clean=True, inv=None, quality_threshold=-np.Inf, taperFraction=0.05, \
                  filterType="bandpass", freq=[0.5, 30.0], corners=6, zerophase=False, outputType='VEL', \
                    miniseed_qc=True, verbose=False, max_dropout=0.0, units='Counts'):
    """
    ...
    if miniseed_qc:
        try:
            ...
            tr.stats["quality_factor"] = _trace_quality_factor(tr, max_dropout=max_dropout) #0 = blank trace, 1 = has some 0s and -1s, 3 = all looks good
    ...
    if verbose:
        print('- clipping')
    _clip_trace(tr) # could add function here to attempt to correct for clipping

    # remove single sample spikes
    if bool_despike:
        if verbose:
            print('- removing spikes')
        _remove_single_sample_spikes(tr)        
        _check_for_spikes(tr)
        if tr.stats.quality_factor < quality_threshold:     
            return False       
    elif verbose:
