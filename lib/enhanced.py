...
from obspy import Stream  # avoid circular import
from metrics import estimate_snr
...
class EnhancedCatalog(Catalog):
    ...
    def plot_eventrate(self, binsize=pd.Timedelta(days=1), time_limits=None):
        """
        Plot histogram and cumulative count of events and their cumulative energy over time.
        """
        df = self.to_dataframe()
        return plot_eventrate(df, binsize=binsize, time_limits=time_limits)
...
def plot_eventrate(df, binsize=pd.Timedelta(days=1), time_limits=None):
    """
    Plot number of events and their cumulative energy in time bins from a DataFrame.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame with at least columns 'datetime' and 'energy'.
    binsize : pd.Timedelta
        Binning resolution (e.g. daily, hourly).
    time_limits : tuple(datetime, datetime)
        Optional time range for plotting.
    ...
    if not dfsum.empty:
        ...
        dfsum.plot.line(ax=axs[0], y='cumcounts', secondary_y=True, style='g', ylabel='Cumulative')
        axs[0].set_xticklabels([])

        dfsum.plot.line(ax=axs[1], y='energy', style='b', ylabel='Energy')
        dfsum.plot.line(ax=axs[1], y='cumenergy', secondary_y=True, style='g', ylabel='Cumulative')
        plt.show()
